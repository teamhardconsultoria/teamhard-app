...
export default function StudentProfile() {
  ...
  const load = async () => {
    const { data: u } = await supabase.from('users').select('name, phone, avatar_url').eq('id', user!.id).single()
  ...
  const handleSaveProfile = async () => {
    ...
    setProfileMsg(null)
    const { error } = await supabase.from('users').update({ name: name.trim(), phone: phone.trim() || null }).eq('id', user!.id)
    if (error) {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', backgroundColor: 'var(--bg)' }}>
      <div style={{ padding: pad, maxWidth: 640 }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text)', margin: '0 0 24px' }}>Meu Perfil</h1>

        {/* Informações */}
        <Section icon={<User size={16} color="#E8FF00" />} title="Informações">
          {/* Avatar */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
            <div style={{ position: 'relative', cursor: 'pointer', flexShrink: 0 }} onClick={() => fileInputRef.current?.click()} title="Alterar foto">
            ...
            <div>
              ...
              <button onClick={() => fileInputRef.current?.click()} style={{ fontSize: 12, color: '#E8FF00', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontWeight: 600 }}>
                Alterar foto
              </button>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Field label="Nome completo *">
              <StyledInput value={name} onChange={setName} placeholder="Seu nome" />
            </Field>
            <Field label="WhatsApp">
              <StyledInput value={phone} onChange={setPhone} placeholder="+55 11 99999-9999" type="tel" />
            </Field>
            <Field label="E-mail">
              <StyledInput value={user?.email ?? ''} onChange={() => {}} placeholder="" disabled />
            </Field>
          </div>

          {profileMsg && (
            <p style={{ fontSize: 13, margin: '14px 0 0', color: profileMsg.type === 'ok' ? '#22c55e' : '#ef4444' }}>{profileMsg.text}</p>
          )}
        ...
        <Section icon={<Lock size={16} color="#E8FF00" />} title="Segurança">
          ...
          {passwordMsg && (
            <p style={{ fontSize: 13, margin: '14px 0 0', color: passwordMsg.type === 'ok' ? '#22c55e' : '#ef4444' }}>{passwordMsg.text}</p>
          )}
          <div style={{ marginTop: 20, display: 'flex', justifyContent: 'flex-end' }}>
